...
def _export_dirname(skill) -> str:
    """Build a readable directory name with full immutable Skill identity."""
    slug = _slugify(skill.name)
    return f"{slug}_v{skill.version}_{skill.skill_id}"
        ...
        try:
            ...
            for relative, body in rendered.items():
                ...
                if existing is not None and existing != hashlib.sha256(body).hexdigest():
                    ...
                    if previous is None or existing != previous[0]:
            ...
            # Receipts identify exact generated bytes across renderer changes.
            # Older stores can enroll files only if they match the current view.
            connection.executemany(
                "INSERT OR REPLACE INTO local_gold_export(path, sha256) VALUES (?, ?)",
                ((relative, hashlib.sha256(body).hexdigest()) for relative, body in rendered.items()),
            )
