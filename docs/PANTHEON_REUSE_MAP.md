...
| `ToolProvider` | Abstract `list_tools`, `call_tool`, `initialize`, and `shutdown` contract | EXTEND | Phase 5 provides a narrow Pantheon-facing `artifact_query` adapter returning only `ArtifactView`; a future provider can expose it without giving agents the store or paths | No |
