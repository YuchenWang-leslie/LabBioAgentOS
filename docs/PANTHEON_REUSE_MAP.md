...
| `ToolProvider` | Abstract `list_tools`, `call_tool`, `initialize`, and `shutdown` contract | EXTEND | Phase 5 provides a narrow `artifact_query` adapter; Phase 6 provides a typed execution boundary that a future provider can wrap without exposing the store, host paths, Docker args, or logs | No |
...
| Tool-result conversion in `Agent._handle_tool_calls` and `pantheon.utils.llm.process_tool_result` | Builds tool messages, removes declared hidden fields, truncates model content, retains `raw_content`, then feeds `content` into the next LLM turn | WRAP | Phase 5/6/8 authorize and filter outputs before any future tool result reaches this conversion; `hidden_to_model` and truncation remain transport features, not the security boundary | No through Phase 8 |
...
| `pantheon.internal.memory.Memory` | Flat message store with metadata, persistence backends, and `execution_context_id` filtering | REUSE | Reuse for team short-term memory only; Phase 8 governed persistent Memory remains a separate LabBio subsystem | No |
| `MemoryManager`, JSON/JSONL backends | Conversation-memory persistence | IGNORE FOR PERSISTENT MEMORY | They do not implement Principal/project authorization, proposal approval, or immutable version lineage | No |
...
| `LearningRuntime`, `SkillStore`, `SkillInjector` | Layered project/global/factory skill discovery, parsing, indexing, and file operations | WRAP | Phase 7/8 keep immutable scoped Gold records and authorization in LabBio; upstream parsing/index mechanics may later present already-authorized procedural memory but cannot own approval, lineage, or ACLs | No |
...
| `BackgroundTaskManager` and Agent background tools | In-process asyncio task launch/adoption, status, cancellation, and notifications | DISABLED FOR LABBIO INVOCATIONS | Factory selects `allow_background_tools=False` so results finish inside the authorized invocation; ordinary await and parallel calls remain. See UPSTREAM_MODIFICATIONS for the published fork revision supplying this generic opt-in API | Agent policy only; manager unchanged |
