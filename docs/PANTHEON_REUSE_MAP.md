...
| Tool-result conversion in `Agent._handle_tool_calls` and `pantheon.utils.llm.process_tool_result` | Builds tool messages, removes declared hidden fields, truncates model content, retains `raw_content`, then feeds `content` into the next LLM turn | WRAP | Exposure policy must run before this point; truncation/`hidden_to_model` alone cannot prevent raw-data retention | No for policy; conditional `pantheon/agent.py` change for structured errors is tracked separately |
...
| `list_agents` injected tool | Discovers peers, excludes self, and excludes ancestors for delegated children | REUSE / EXTEND | Preserve discovery; add policy-filtered allowed-agent view in a LabBio extension | Prefer no; conditional `pantheon/team/pantheon.py` watch item |
| `call_agent` injected tool | Validates target/instruction, constructs delegation metadata/context, creates child memory, runs target, forwards child events, and returns child content | REUSE / EXTEND | It already implements the required collaboration chain; policy must gate it rather than replace it | Prefer no; conditional watch item |
